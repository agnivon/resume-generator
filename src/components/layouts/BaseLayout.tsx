...
const BaseLayout = async ({ children }: { children: React.ReactNode }) => {
  return (
    <>
      <AppProvider>
        <MotionDiv
          className="hidden xl:flex w-full min-h-screen overflow-y-auto overflow-x-hidden print-initial"
          id="rootContainer"
        >
          <NProgressBar />
          <div className="min-h-screen flex w-full bg-white dark:bg-gray-900 text-black dark:text-white overflow-y-auto print-bg-color-reset">
            {children}
          </div>
        </MotionDiv>
        <ResponsiveMessage />
